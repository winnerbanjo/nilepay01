      ...
      <div className="bg-white border border-nile-border rounded-[32px] shadow-sm overflow-hidden animate-fade-in-up">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-nile-border">
            <thead className="bg-slate-50/70 text-left text-xs font-bold text-nile-muted uppercase tracking-wider">
              <tr>
            ...
            <tbody className="bg-white divide-y divide-nile-border text-sm">
              {sortedApps.length === 0 ? (
                <tr>
                  <td colSpan="8" className="px-6 py-20 text-center text-nile-muted italic bg-slate-50/10">
                ...
                sortedApps.map((app) => (
                  <tr key={app.id} className="hover:bg-slate-50/50 transition duration-150 group">
                    <td className="px-6 py-4">
                      <div className="font-bold text-nile-dark group-hover:text-nile-darkgreen transition duration-150">{app.businessName || 'Pending Onboarding'}</div>
                      <div className="text-xs text-nile-muted font-normal">{app.merchantName || 'New Merchant'} ({app.kycData?.email || 'No email'})</div>
                      <div className="text-[10px] text-slate-400 mt-0.5 font-semibold">ID: {app.id}</div>
                    </td>
                    <td className="px-6 py-4 font-semibold text-nile-darkgreen text-xs">
                      {app.website === 'pending.nile.ng' ? <span className="text-slate-400 italic">Pending</span> : app.website}
                    </td>
                    ...
                    <td className="px-6 py-4 text-xs text-nile-dark font-semibold">
                      {app.country}
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${
                        app.status === 'Payment Activated' ? 'bg-green-50 text-green-700 border-green-200 shadow-sm' :
                        app.status === 'Approved' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                        app.status === 'Submitted' ? 'bg-blue-50 text-blue-700 border-blue-200 shadow-sm animate-pulse' :
                    ...
                    <td className="px-6 py-4 text-xs font-medium">
                      <div className="text-nile-dark font-semibold">{getSignupDateString(app)}</div>
                      <div className="text-[10px] text-nile-muted mt-0.5">Updated: {getLastUpdatedString(app)}</div>
                    </td>
                    <td className="px-6 py-4 text-xs text-nile-dark font-semibold">
                      {app.assignedReviewer ? (
                        <span className="flex items-center gap-1.5">
                          <span className="w-5 h-5 rounded-full bg-nile-darkgreen text-white font-bold text-[9px] flex items-center justify-center">
                    ...
                    <td className="px-6 py-4 text-center">
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-4 py-2 bg-nile-softmint text-nile-darkgreen hover:bg-[#c9ebd6] text-xs font-bold rounded-xl transition-all duration-200 shadow-sm hover:shadow hover:-translate-y-0.5 btn-stripe-style"
                      >
                        <Eye size={13} />
                        <span>Audit KYC</span>
                      </button>
